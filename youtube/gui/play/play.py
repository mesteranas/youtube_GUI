...
class PlayThread(qt2.QRunnable):
    ...
    def run(self):
        ...
        # Using pafy to fetch video info
        video_info = pafy.new(self.url)
        video_data = {
            'title': video_info.title,
            'uploader': video_info.author,
            'webpage_url': video_info.watchv_url,
            'id': video_info.videoid,
            'description': video_info.description,
            'channel_id': video_info.username
        }
        self.objects.finish.emit(video_data)

        # Emit the stream URL for video or audio
        if self.type == 0:
            self.objects.url.emit(video_info.getbest().url)
        else:
            self.objects.url.emit(video_info.getbestaudio().url)
...
class Play(qt.QDialog):
    def __init__(self, p, videoURL, type):
        ...
        self.showFullScreen()
        self.video = None
        self.stoppedPosition = 0
        self.gotoStoppedPosition = False
        ...
        comments.triggered.connect(lambda: self.on_comments(self.video['id']))
    ...
    def closeEvent(self, event):
        gui.history.historyJsonControl.save(self.video['title'] + _("by") + self.video['uploader'], self.position, self.video['webpage_url'])
        self.media.stop()
        self.close()
    ...
    def on_url(self, url):
        ...
        state, position, url = gui.history.historyJsonControl.get(self.video['title'] + _("by") + self.video['uploader'])
        self.stoppedPosition = position
        self.media.play()
    ...
    def on_favorite(self, index):
        text = self.video['title'] + _("by") + self.video['uploader']
        ...
        if index == 0:
            ...
            if data.get(text):
                ...
                data[text] = {"url": self.video['webpage_url'], "position": 0}
            gui.favorite.favoriteJsonControl.save("videos", data)

    def on_position_changed(self, position):
        if position == 0:
            return
        if self.gotoStoppedPosition == False and position >= 3000:
            self.media.setPosition(self.stoppedPosition)
            self.gotoStoppedPosition = True
        self.position = position

    def on_download(self):
        self.media.pause()
        gui.download.DownloadGUI(self, self.video['webpage_url']).exec()

    def on_download_subtitles(self):
        self.media.pause()
        gui.download.DownloadSubtitles(self, self.video['id']).exec()
