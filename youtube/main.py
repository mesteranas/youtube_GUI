...
class main (qt.QMainWindow):
    def __init__(self):
        ...
        self.search.clicked.connect(lambda:gui.search.Search(self).exec())
        SearchIcon=qt1.QIcon("data/icons/youtube-search.png")
        self.search.setIcon(SearchIcon)
        layout.addWidget(self.search)
        self.open=qt.QPushButton(_("open"))
        self.open.setDefault(True)
        self.open.clicked.connect(lambda:gui.Open(self).exec())
        layout.addWidget(self.open)
        self.favorites=qt.QPushButton(_("favorites"))
        self.favorites.setDefault(True)
        self.favorites.clicked.connect(lambda:gui.favorite.FavoritGUI(self).exec())
        layout.addWidget(self.favorites)
        self.history=qt.QPushButton(_("history"))
        self.history.setDefault(True)
        self.history.clicked.connect(lambda:gui.history.HistoryGUI(self).exec())
